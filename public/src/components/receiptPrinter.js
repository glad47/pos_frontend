...
const generateReceiptHTML = (order, companyInfo) => {
  ...
  const { orderNumber = '', subtotal = 0, taxAmount = 0, totalAmount = 0, discountAmount = 0, paymentMethod = 'CASH', cashierName = 'Cashier', createdAt = new Date().toISOString(), change = 0, amountPaid = 0, loyaltySections = [], remainingItems = [], customerName = null, customerPhone = null, customerVat = null, isReturn = false, originalOrderNumber = '', returnReason = '' } = order;
  ...
  let invoiceTitle, invoiceTitleEn;
  if (isReturn) {
    invoiceTitle = 'إشعار دائن ضريبي';
    invoiceTitleEn = isTaxInvoice ? 'Tax Credit Note' : 'Simplified Tax Credit Note';
    if (!isTaxInvoice) invoiceTitle = 'إشعار دائن ضريبي مبسط';
  } else {
    invoiceTitle = isTaxInvoice ? 'فاتورة ضريبية' : 'فاتورة ضريبية مبسطة';
    invoiceTitleEn = isTaxInvoice ? 'Tax Invoice' : 'Simplified Tax Invoice';
  }
...
` : '';
  ...
  const otherHeader = (loyaltySections.length > 0 && remainingItems.length > 0)
    ...
    : (isReturn && remainingItems.length > 0)
    ? '<tr style="border-top:0.5px dashed #999;"><td colspan="4" style="padding:5pt 2pt 2pt;text-align:right;font-weight:bold;font-size:7.5pt;">Return Items / اصناف مرتجعة</td></tr>'
    : '';
...
${isReturn ? `
<table style="margin-bottom:5pt;font-size:7pt;border-top:0.5px dashed #999;padding-top:5pt;">
  <tr><td colspan="3" style="text-align:center;padding:3pt 0;font-weight:bold;font-size:8pt;color:#c00;">↩️ RETURN / مرتجع</td></tr>
  <tr><td class="en" style="padding-top:2pt;">Original Order:</td><td class="center bold" style="padding-top:2pt;font-size:7pt;"><span class="num">${originalOrderNumber}</span></td><td style="padding-top:2pt;text-align:right;">الفاتورة الأصلية:</td></tr>
  ${returnReason ? '<tr><td class="en" style="padding-top:2pt;">Reason:</td><td class="center" style="padding-top:2pt;font-size:7pt;">' + returnReason + '</td><td style="padding-top:2pt;text-align:right;">السبب:</td></tr>' : ''}
</table>
` : ''}
...
<table style="font-size:8pt;">
  ...
  <tr style="border-top:1px solid #000;"><td style="padding:5pt 0;"><span class="bold" style="font-size:10pt;">${isReturn ? 'مبلغ الاسترداد' : 'المجموع النهائي'}</span> <span class="bold en" style="font-size:8pt;">/ ${isReturn ? 'Refund Total' : 'Total'}</span></td><td class="center bold" style="padding:5pt 0;font-size:11pt;">${fc(totalAmount)}</td></tr>
</table>
</div>

<!-- Payment -->
<table style="font-size:7.5pt;margin-bottom:6pt;border-top:0.5px dashed #999;padding-top:5pt;">
  <tr><td style="padding:3pt 0;"><span class="bold">${isReturn ? 'طريقة الاسترداد' : 'طريقة الدفع'}</span> <span class="en" style="font-size:6.5pt;">/ ${isReturn ? 'Refund Method' : 'Payment'}</span></td><td class="center bold" style="padding:3pt 0;">${paymentMethod === 'CASH' ? 'نقدي / Cash' : 'بطاقة / Card'}</td></tr>
  <tr><td style="padding:3pt 0;"><span class="bold">${isReturn ? 'مبلغ الاسترداد' : 'المبلغ المستلم'}</span> <span class="en" style="font-size:6.5pt;">/ ${isReturn ? 'Refund Amount' : 'Paid'}</span></td><td class="center bold" style="padding:3pt 0;">${fc(actualAmountPaid)}</td></tr>
  ${!isReturn && actualChange > 0 ? '<tr><td style="padding:3pt 0;"><span class="bold">الباقي</span> <span class="en" style="font-size:6.5pt;">/ Change</span></td><td class="center bold" style="padding:3pt 0;">'+fc(actualChange)+'</td></tr>' : ''}
</table>
